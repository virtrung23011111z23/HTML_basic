...
import { IoIosArrowDown, IoMdAdd, IoIosRemove, IoMdCloseCircleOutline, IoMdArrowRoundBack, IoMdClose } from "react-icons/io";
...
export default function Cart() {
    ...
    useEffect(() => {
        setVillageId("");
        const distrisPart = dataCity.find(p => p.codename === cityId);
        setDistrict(distrisPart ? distrisPart.districts : []);
    }, [cityId, dataCity]);

    useEffect(() => {
        setVillageId("");
        const villagePart = district.find(p => p.codename === districtId);
                            ...
                            <div className="form__input-box">
                                <div className="form__select-box active">
                                    <select id="select-city" name="select-city" onChange={(e) => setCityId(e.target.value)} >
                                        ...
                                        {dataCity.map((city, i) =>
                                            <option key={i} value={city.codename}>{city.name}</option>
                                        )}
                                    </select>
                                ...
                                <div className={`form__select-box ${cityId ? 'active' : ''}`}>
                                    <select id="select-district" name="select-district" onChange={(e) => setDistrictId(e.target.value)}>
                                        ...
                                        {district.map((district) =>
                                            <option key={district.codename} value={district.codename}>{district.name}</option>)}
                                    </select>
                                    <IoIosArrowDown className="form__select-icon" size={20} />
                                </div>
                                <div className={`form__select-box ${districtId ? 'active' : ''}`}>
                                    <select id="select-village" name="select-village" onChange={(e) => setVillageId(e.target.value)}>
                                        ...
                                        {village.map((village) =>
                                            <option key={village.codename} value={village.codename}>{village.name}</option>)}
                                    </select>
                                    <IoIosArrowDown className="form__select-icon" size={20} />
                                </div>
                            </div>
                            <input type="text" name="address" className="form__adress" placeholder="Địa chỉ" onBlur={handleBluer} />
                            {errMesagge.address &&
